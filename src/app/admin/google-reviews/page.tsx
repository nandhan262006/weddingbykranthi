...
import { HiPlus, HiPencilSquare, HiTrash, HiStar } from "react-icons/hi2";
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/30 text-sm mt-1">Manage your customer reviews</p>
        </div>
        <Link
          href="/admin/google-reviews/new"
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.06]">
                <th className="text-left px-5 py-3 text-white/30 text-xs font-medium uppercase tracking-wider">Name</th>
                <th className="text-left px-5 py-3 text-white/30 text-xs font-medium uppercase tracking-wider">Rating</th>
            ...
            <tbody>
              {items.map((r) => (
                <tr key={r.id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                  <td className="px-5 py-3.5 text-white text-sm font-medium">{r.name}</td>
                  <td className="px-5 py-3.5 text-yellow-400 text-sm">
                    <HiStar className="w-4 h-4 inline mr-1" />
                    {r.rating}/5
                  </td>
                  <td className="px-5 py-3.5 text-white/40 text-sm">{r.date || "—"}</td>
                  <td className="px-5 py-3.5">
                    <span className={`text-[10px] px-2 py-1 rounded-full font-medium ${
                      r.isActive ? "bg-green-500/15 text-green-300" : "bg-white/5 text-white/30"
                    }`}>
              ...
              {items.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-5 py-12 text-center text-white/20 text-sm">No reviews yet</td>
                </tr>
              )}
            </tbody>
          </table>
